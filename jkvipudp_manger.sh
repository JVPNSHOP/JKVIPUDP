...
# Function to check and open firewall port
open_firewall_port() {
    local port=$1
    ...
    if command -v ufw >/dev/null 2>&1 && ufw status | grep -q "active"; then
        if ! ufw status | grep -q "$port/tcp"; then
            ...
            echo -e "${GREEN}✓ UFW firewall: Port $port opened${NC}"
        fi
    fi
    ...
    if command -v iptables >/dev/null 2>&1; then
        if ! iptables -L -n | grep -q "tcp.*$port"; then
...
toggle_web_server() {
    if is_web_enabled; then
        ...
        if [[ "$confirm" == "yes" ]]; then
            ...
            # Open firewall port 8088
            open_firewall_port 8088
...
<body>
    ...
    <script>
        ...
        function updateDashboard(data) {
            ...
            const apiJsonUrl = `http://${data.server_ip}:8088/udpserver/online_app`;
            const apiTextUrl = `http://${data.server_ip}:8088/udpserver/online`;
            const dashboardUrl = `http://${data.server_ip}:8088/udpserver/`;
                    ...
                    <div class="api-links">
                        <div class="api-link-card">
                            <div class="api-link-header">
                                ...
                                <div class="api-link-title">📊 Dashboard</div>
                            </div>
                            <div class="api-url-box">${dashboardUrl}</div>
                            <div class="api-buttons">
                                <a href="${dashboardUrl}" target="_blank" class="btn">
                                    <span class="material-icons">open_in_new</span>
                                    Open Dashboard
                                </a>
                                <button class="btn" onclick="copyToClipboard('${dashboardUrl}', this)">
...
HTMLEOF
            ...
            if nginx -t >/dev/null 2>&1; then
                ...
                echo -e "${CYAN}Dashboard: http://${server_ip}:8088/udpserver/${NC}"
                echo -e "${CYAN}API JSON:  http://${server_ip}:8088/udpserver/online_app${NC}"
                echo -e "${CYAN}API TEXT:  http://${server_ip}:8088/udpserver/online${NC}"
                echo -e "${YELLOW}Note: Port 8088 is now accessible from external networks${NC}"
            else
