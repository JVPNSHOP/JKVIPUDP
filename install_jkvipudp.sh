...
show_usage_and_exit() {
    echo
    echo -e "\t$(tbold)$SCRIPT_NAME$(treset) - JKVIPUDP server install script"
    echo
    echo -e "Usage:"
    echo
    echo -e "$(tbold)Install JKVIPUDP$(treset)"
    ...
    echo -e "\t-l, --local <file>\tInstall specified JKVIPUDP binary instead of download it."
    echo -e "\t--version <version>\tInstall specified version instead of the latest."
    echo
    echo -e "$(tbold)Remove JKVIPUDP$(treset)"
    echo -e "\t$0 --remove"
    echo
...
perform_install_manager_script() {
    local _manager_script="/usr/local/bin/jkvipudp_manager.sh"
    local _symlink_path="/usr/local/bin/jkvipudp"
...
perform_install() {
    ...
    if [[ -n "$_is_fresh_install" ]]; then
        echo
        echo -e "$(tbold)Congratulations! JKVIPUDP has been successfully installed on your server.$(treset)"
        echo "Use 'jueudp' command to access the manager."

        echo
        echo -e "$(tbold)Client  Jue VPN APK:$(treset)"
        echo -e "$(tblue)https://play.google.com/store/apps/details?id=com.myjvpndev.net$(treset)"
        echo
        echo -e "Follow me!"
        echo
        echo -e "\t+ Check out my website at $(tblue)https://jvpnshop.site/$(treset)"
        echo -e "\t+ Follow me on Telegram: $(tblue)https://t.me/Pussy1990$(treset)"
        echo -e "\t+ Contact me on Messenger: $(tblue)https://m.me/juehtet2025/$(treset)"
        echo
    else
        restart_running_services
        start_services
        echo
        echo -e "$(tbold)JKVIPUDP has been successfully updated to $VERSION.$(treset)"
        echo
    fi
}
...
perform_remove() {
    ...
    echo
    echo -e "$(tbold)Congratulations! JKVIPUDP has been successfully removed from your server.$(treset)"
    echo
